...
const isQueueLocked = async () => {
  try {
    ...
    logger.errorWrapper("Failed to check queue lock state", err);
  ...
  async execute(interaction) {
    ...
    try {
      if (subcommand === "list") {
        ...
        const list = queuedUsers
          .map(
            (player, index) =>
              `${index + 1}. <@${player.id}> - **${
                player.platform?.toUpperCase() || "UNKNOWN"
              }**`
          )
          .join("\n");
      ...
      if (["lock", "unlock"].includes(subcommand)) {
        ...
        logger.info(
          `🔒 Queue ${lockValue ? "locked" : "unlocked"} by ${
      ...
      if (subcommand === "number") {
        ...
        for (const row of queueStats) {
          const platform = row.platform?.toUpperCase() || "UNKNOWN";
          reply += `🔹 **${platform}**: ${row.count} players\n`;
        }
      ...
      logger.errorWrapper("❌ Error executing /queue command", error, {
